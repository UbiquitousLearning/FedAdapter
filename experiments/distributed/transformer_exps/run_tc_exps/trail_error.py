...
args.max_round = 1000
args.expand = 0.5 # time_thereshold的膨胀系数，actual time_threshold is time_threshold * (expand * depth + 1). 越深的层应该更稳定，可以减少trial频率
...
while freeze_layers[1][-1] < args.max_round: # max_round = 1000
    ...
    hp_shallow = 'FedAvg "uniform" 0.1 1 ' + str(round_shallow) + ' 5 ' + remove_space(str(freeze_layers[0]).replace(',','.')+','+str(freeze_layers[1]).replace(',','.')+','+str(freeze_layers[2]))+" "+str(args.depth)+" "+str(args.time_threshold) # linux 读取输入的时候以，为分隔符，需要替换掉

    freeze_layers[2] = depth_deep
    hp_deep = 'FedAvg "uniform" 0.1 1 ' + str(round_deep) + ' 5 ' + remove_space(str(freeze_layers[0]).replace(',','.')+','+str(freeze_layers[1]).replace(',','.')+','+str(freeze_layers[2]))+" "+str(args.depth)+" "+str(args.time_threshold)
