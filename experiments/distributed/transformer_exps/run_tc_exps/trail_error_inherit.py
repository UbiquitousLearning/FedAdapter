...
while freeze_layers[1][-1] < args.max_round: # max_round = 1000
    ...
    hp_shallow = 'FedAvg "uniform" 0.1 1 ' + str(delta_round_shallow) + ' 5 ' + remove_space(str([freeze_layers[2]]).replace(',','.')+','+str([-1]).replace(',','.')+','+str(freeze_layers[2]))+" "+str(args.depth)+" "+str(args.time_threshold) # linux 读取输入的时候以，为分隔符，需要替换掉

    freeze_layers[2] = depth_deep
    hp_deep = 'FedAvg "uniform" 0.1 1 ' + str(delta_round_deep) + ' 5 ' + remove_space(str([freeze_layers[2]]).replace(',','.')+','+str([-1]).replace(',','.')+','+str(freeze_layers[2]))+" "+str(args.depth)+" "+str(args.time_threshold)

    os.system("mkdir ./tmp/; touch ./tmp/fedml-shallow-{args.depth}-{args.time_threshold}; touch ./tmp/fedml-deep-{args.depth}-{args.time_threshold}; mkdir -p ./results/BERT/Trail-{args.depth}-{args.time_threshold}/size-32".format(args=args))
    ...
    print('nohup sh run_text_classification_freeze_shallow.sh '
                '{args.hp} '
                '> ./results/BERT/Trail-{args.depth}-{args.time_threshold}/size-32/fednlp_tc_shallow_{args.run_id}.log 2>&1 &'.format(args=args))
    os.system('nohup sh run_text_classification_freeze_shallow.sh '
                '{args.hp} '
                '> ./results/BERT/Trail-{args.depth}-{args.time_threshold}/size-32/fednlp_tc_shallow_{args.run_id}.log 2>&1 &'.format(args=args))
    ...
    print('nohup sh run_text_classification_freeze_deep.sh '
                '{args.hp} '
                '> ./results/BERT/Trail-{args.depth}-{args.time_threshold}/size-32/fednlp_tc_deep_{args.run_id}.log 2>&1 &'.format(args=args))
    os.system('nohup sh run_text_classification_freeze_deep.sh '
                '{args.hp} '
                '> ./results/BERT/Trail-{args.depth}-{args.time_threshold}/size-32/fednlp_tc_deep_{args.run_id}.log 2>&1 &'.format(args=args))
