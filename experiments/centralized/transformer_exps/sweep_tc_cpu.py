...
os.system("cp ./main_tc_cpu.py ./main_tc.py ")
...
for hp in hps[::-1]:
    ...
    os.system("mkdir ./tmp/; mkdir ./tmp/CPU/")
    os.system('nohup sh run_text_classification.sh '
            '%s '
            '> ./tmp/CPU/origin-fednlp_tc_freeze_%s.log 2>&1' % (hp, layers))
...
for hp in hps[::-1]:
    ...
    layers = hp.split(" ")[-1]
    logging.info("hp = %s" % args.hp)
    os.system("mkdir ./tmp/; mdkir./tmp/CPU/")
    os.system('nohup sh run_text_classification_sweep.sh '
              '%s '
              '> ./tmp/CPU/adapter-fednlp_tc_freeze_%s.log 2>&1' % (hp, layers))
